...
class TestTrustOpsSystem(unittest.TestCase):
    ...
    def test_edge_case_empty_queue_step(self):
        ...
        self.assertEqual(step_result["reward"], 0.3)
    ...
    def test_score_never_zero_or_one(self):
        """Ensures clamped scores are strictly in (0.3, 0.7)."""
        self.assertEqual(normalize_score(0.0, 1.0), 0.3)
        self.assertEqual(normalize_score(1.0, 1.0), 0.7)
        self.assertEqual(normalize_score(-0.5, 1.0), 0.3)
        self.assertEqual(normalize_score(1.5, 1.0), 0.7)
        self.assertEqual(normalize_score(0.5, 1.0), 0.5)

    def test_grader_scores_in_valid_range(self):
        """Run all graders and ensure scores are in (0.3, 0.7)."""
        ...
        for content in CONTENT_BANK:
            ...
            self.assertGreaterEqual(rec, 0.3, f"{content.id} score below 0.3")
            self.assertLessEqual(rec, 0.7, f"{content.id} score above 0.7")
            ...
            self.assertGreaterEqual(rec2, 0.3, f"{content.id} wrong-action score below 0.3")
            self.assertLessEqual(rec2, 0.7, f"{content.id} wrong-action score above 0.7")
    ...
    def test_connection_failure_simulation(self, mock_openai):
        ...
        for task_name, task_result in results["tasks"].items():
            self.assertGreaterEqual(task_result["score"], 0.3, f"{task_name} score too low")
            self.assertLessEqual(task_result["score"], 0.7, f"{task_name} score too high")
        self.assertGreaterEqual(results["score"], 0.3)
        self.assertLessEqual(results["score"], 0.7)
